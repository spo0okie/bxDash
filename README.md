...
* loadData - загрузить из JSON-структуры Bitrix (вызывается из init и при WS-обновлениях)
* recalcTime - пересчитать вычисляемые поля времени (`t`, `deadlineStr`, `closedDateStr`, `isClosed`/`isOpen`)
* update - изменение параметров на фронтенде (если указать save, то и в бэкэнде)
* save - сохранение изменений в бэк
* movePosition - задать новые параметры элемента (после d-n-d). Спрашивает подтверждение пользователя, потом save в бэк
* delete - удалить (синхронизирует с бэком)

> Привязка к интервалу/периоду больше НЕ хранится на DashItem (нет `intervalId/periodId/findInterval/setInterval/...`).
> Период сам решает, какие элементы в него попадают, через computed `PeriodItem._pick` (фильтрация по `t in [start, end]`).
> Это даёт реактивную перепривязку без императивных `unsetX/setX` при смене даты, deadline или статуса.
...
* Элемент может упасть в ячейку или на другой элемент в ячейке
  * При падении на другой элемент
    ...
    * Определяем sort-index'ы элементов между которыми уронили (для этого ячейка передаёт в DnD-данные текущий список)
    * Ставим себе индекс посередине между соседями
  * При падении на ячейку добавляем элемент в конец списка (sort index ниже самого нижнего)
* Логика drop сосредоточена в [`Helpers/DndHelper.jsx`](src/Helpers/DndHelper.jsx) — функции `computeNewSort` (новый sorting), `buildMoveParams` (сборка параметров), `dashItemsDrop` (точка входа). Drop-on-item и drop-on-cell делят общий путь, различаются только стратегией `computeNewSort`.

таким образом

* в DnD-данных ячейки лежит признак `type:'cell'`, плюс:
  * `cell.id` — ключ ячейки (period.start + '/' + userId + '/' + priority)
  * `cell.user`, `cell.t`, `cell.dropT`, `cell.priority`, `cell.maxSorting`, `cell.minSorting`
  * `items` — текущий отсортированный список элементов ячейки (для расчёта новых sort-индексов)
* в DnD-данных карточки лежит признак `type:'item'`, плюс:
  * `item` — сам DashItem
  * `index` — порядок в списке ячейки (для определения соседей при drop-on-item)
  * `cell` — ссылка на ту же структуру ячейки
